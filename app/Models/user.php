<?php
	...
	class User {
		static function insert($input) {
			$data = array($input['name'],$input['email'],Hash::make($input['password']));
			$result = DB::insert("INSERT INTO users (name, email, password) VALUES (?, ?, ?)", $data);
			return $result;
		}
		static function update($input) {
			if($input['password']) {
				...
				$data = array($input['name'],$input['email'],$input['phone'],$input['address']);
